...
names(dataset_stats2) <- c("total intervenciones", "total párrafos", "total palabras", 
                           "intervención/párrafos",
                           "intervención/palabras")
...
ggsave(plot = plot_votaciones, filename = "cuadros_tesis/plot_n_votaciones.png")
...
polarity_scatter_plot <-  polarity_scatter %>% 
  ...
  geom_point(size = 1.5) +
  labs(caption = "Fuente: Elaboración propia con datos de la biblioteca del Congreso Nacional") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.text = element_text(size = 14),
...
ggsave(plot = polarity_scatter_plot, filename = "cuadros_tesis/polarity_scatter_plot.png", width = 10, height = 6)
...
El modelo recibe como entrada un texto y una serie de tópicos considerados relevantes. La respuesta consiste en un vector que contiene la probabilidad que la red le asigna a cada tópico. Para cada texto se seleccionó el tópico con probabilidad más alta, el cual se usó como etiqueta. 
Los tópicos considerados fueron: 1) salud, 2) educación, 3) deporte, 4) medioambiente, 5) impuestos, 6) cultura, 7) pensiones, 8) sindicalismo, 9) transporte, 10) familia y 11) aborto. 
...
Al observar los datos en perspectiva histórica se observa que 
